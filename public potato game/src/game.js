...
// Game variables
let userId = null;
let streamerId = "public";  // Fixed streamerId for public version
let user_name = "";
let lastDropTime = 0;
...
onAuthStateChanged(auth, (user) => {
  if (user) {
    ...
    // Set username based on auth info
    if (user.isAnonymous) {
      // For guest users, create a random name or allow them to set one
      const storedGuestName = localStorage.getItem('guestUsername');
      if (storedGuestName) {
        user_name = storedGuestName;
      } else {
        user_name = "Guest_" + Math.floor(Math.random() * 10000);
        localStorage.setItem('guestUsername', user_name);
      }
    } else {
      // For registered users, use the selected username from localStorage
      const storedUsername = localStorage.getItem('registeredUsername');
      if (storedUsername) {
        user_name = storedUsername;
      } else {
        user_name = "User_" + Math.floor(Math.random() * 10000);
        localStorage.setItem('registeredUsername', user_name);
      }
    }
    
    // Update UI
    document.getElementById('login-screen').style.display = 'none';
    document.getElementById('pause-menu').setAttribute('data-username', `Playing as: ${user_name}`);
    
    // Check if user exists in database
    checkUserAndStart(db, userId, streamerId);
  } else {
...
async function checkUserAndStart(db, userId, streamerId) {
  ...
  try {
    ...
    if (docSnap.exists()) {
      console.log("User exists, starting game...");
      if (docSnap.data().username) {
        user_name = docSnap.data().username;
        document.getElementById('current-user').textContent = `Playing as: ${user_name}`;
      }
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Set up event handlers for auth
  document.getElementById('login-button').addEventListener('click', async () => {
    ...
    try {
      ...
      // Get the email for this username to login with Firebase
  ...
  document.getElementById('register-button').addEventListener('click', async () => {
    ...
    try {
      // Generate a unique email based on username for Firebase (which requires email)
      const email = `${username.replace(/[^a-zA-Z0-9]/g, '')}${Date.now()}@potatogame.com`;
      
      // Store username in localStorage
      localStorage.setItem('registeredUsername', username);
      
      // Create user with generated email/password
      const userCredential = await createUserWithEmailAndPassword(auth, email, password);
      
      // Store the username in Firestore for this user
  ...
  document.getElementById('guest-button').addEventListener('click', async () => {
    try {
      // Create a random username for guest
      const guestName = "Guest_" + Math.floor(Math.random() * 10000);
      localStorage.setItem('guestUsername', guestName);
      
      await signInAnonymously(auth);
      // Auth state listener will handle the post-login flow
    } catch (error) {
  ...
  // Handle logout button in pause menu
  document.getElementById('logout-button-pause').addEventListener('click', async () => {
    try {
      document.getElementById('pause-screen').style.display = 'none';
      await signOut(auth);
      // Auth state listener will handle the post-logout flow
    } catch (error) {
  ...
  const Game = {
    ...
    loadHighscores: async function () {
      ...
      console.log("Loading scores for streamerId: " + streamerId);
    ...
    resetGame: function() {
      ...
      // Stop the runner to pause physics updates
      Matter.Runner.stop(runner);
      
      // Clear all bodies from the world (including static ones)
      Matter.World.clear(engine.world, false);
      
      // Reset engine completely to fix physics lag
      Matter.Engine.clear(engine);
      
      // Reset game state variables
      Game.stateIndex = GAME.STATES.MENU;
      Game.score = 0;
      Game.fruitsMerged = Array(Game.fruitSizes.length).fill(0);
      ...
      // Remove preview ball explicitly
      if (Game.elements.previewBall) {
        Matter.Composite.remove(engine.world, Game.elements.previewBall);
        Game.elements.previewBall = null;
      }
      
      // Clear next fruit image
      Game.elements.nextFruitImg.src = '';
      setTimeout(() => {
        Game.elements.nextFruitImg.src = './assets/img/circle0.png';
      }, 50);
      ...
      // Clear any animations or warnings
      Game.elements.warning.classList.remove('flashing');
      Game.elements.canvas.classList.remove('flash');
      
      // Reset and disable buttons until game starts
      document.getElementById('pause-button').disabled = true;
      document.getElementById('refresh-button').disabled = true;
      
      // Recreate the mouse constraint with proper settings
      if (mouseConstraint) {
      ...
      const mouseConstraint = Matter.MouseConstraint.create(engine, {
        ...
        collisionFilter: {
          mask: 0x0001 // Only interact with category 1 (menu buttons)
        }
      });
      render.mouse = mouse;
      ...
      // Re-register the menu mouse handler
      const menuMouseDown = function() {
        if (mouseConstraint.body === null || mouseConstraint.body?.label !== 'btn-start') {
          return;
        }
      ...
      // Reset render properly
      Matter.Render.lookAt(render, {
        min: { x: 0, y: 0 },
        max: { x: Game.width, y: Game.height }
      });
      
      // Start the runner again with fixed timestep to prevent physics lag
      runner.delta = 1000 / 60; // 60 FPS
      Matter.Runner.run(runner, engine);
      
      // Reset the next fruit for when game starts
...
// Game-end link handler (try again button)
document.getElementById('game-end-link').addEventListener('click', function(e) {
  e.preventDefault();
  
  // Ensure runner is in a clean state before resetting
  runner.enabled = true;
  
  // Force game into menu state
  Game.stateIndex = GAME.STATES.MENU;
  
  // Clear the end screen
  Game.elements.end.style.display = 'none';
  
  // Reset the engine timing to normal
  engine.timing.timeScale = 1;
  
  // Now call resetGame which should work as it does with the refresh button
  Game.resetGame();
});
  ...
  // Add the responsive canvas resizing
  const resizeCanvas = () => {
    ...
    if (screenWidth * (Game.height / Game.width) > screenHeight) {
      newHeight = Math.min(Game.height, screenHeight - 100);
      newWidth = newHeight * (Game.width / Game.height);
      scaleUI = newHeight / Game.height;
    } else {
      newWidth = Math.min(Game.width, screenWidth - 40);
